namespace Destination.Application.Common.Behaviours;
...
[SuppressMessage("Code Smell", "S1144:Unused private methods should be removed",
    ...
    private static ValidationError CreateValidationError(ValidationFailure[] validationFailures) =>
        new([
            .. validationFailures.Select(f =>
                Error.Problem(f.PropertyName, f.ErrorMessage)
            )
        ]);
